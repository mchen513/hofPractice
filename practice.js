...
var glutenFree = function (desserts) {
  return _.map(desserts, function(dessert, index, collection) {
    dessert.glutenFree = _.indexOf(dessert.ingredients, 'flour') === -1;
    return dessert;
  });
};
...
var applyCoupon = function (groceries, coupon) {
  return _.map(groceries, function(grocery, index, collection) {
    var currentPriceInCents = 100 * Number(grocery.price.substring(1));
    var salePriceInCents = currentPriceInCents * (1 - coupon);
    grocery.salePrice = '$' + (salePriceInCents / 100).toFixed(2);
    return grocery;
  });
};
...
var sumTotal = function (products) {
  return _.reduce(products, function(accumulator, product) {
    return accumulator + Number(product.price.substring(1));
  }, 0);
};
